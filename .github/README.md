...
| Runs Home Assistant OS and it's add-ons like deCONZ, MariaDB, SMB & SSH.  | Zigbee Universal USB gateway used to connect all my Zigbee nodes.  | Contains a Homebridge instance for any devices Home Assistant doesn't support.  | Primarily in use to control the entertainment area in conjuction with the HDMI Sync Box. Also used for OTAU Hue devices.  | In use for the Ambilight behind the tv and the rest of the entertainment area  | In use to connect to SwitchBot Curtains, runs through HomeBridge to Home Assistant.  | In use as HomeKit Home Hubs to run location automations and control the Home app when away |
